...
function EvidenceDrawer({ finding, close }: { finding: Finding; close: () => void }) {
  ...
  return <aside className="evidence-drawer"><div className="panel-title"><div><p className="eyebrow">EVIDENCE {finding.findingId}</p><h2>{finding.serviceId}</h2></div><button className="secondary" onClick={close}>Close</button></div><div className="contract-grid"><p><strong>{finding.deploymentId}</strong>Deployment</p><p><strong>{finding.environment}</strong>Environment · {finding.criticality}</p><p><strong>{finding.snapshotId}</strong>Immutable snapshot</p><p><strong>{finding.repositoryId}</strong>Repository</p><p><strong>{finding.affectedPackageName}@{finding.affectedVersion}</strong>Exact affected version</p><p><strong>{new Date(finding.firstExposedAt).toLocaleString()}</strong>First exposed</p><p><strong>{finding.lastExposedAt === null ? "Still active" : new Date(finding.lastExposedAt).toLocaleString()}</strong>Exposure end</p><p><strong>{reachabilityName(finding.reachability)}</strong>Reachability classification</p></div>{path !== undefined && <div className="path-list"><h3>Complete dependency path</h3><ol>{path.nodes.map((node) => <li key={node.resolutionId}><button onClick={() => void navigator.clipboard.writeText(node.resolutionId)}>{node.packageName}@{node.version}</button><small>{node.resolutionId}</small></li>)}</ol></div>}<h3>Evidence references</h3><div className="evidence-row">{finding.evidenceRefs.map((reference) => <button key={reference} onClick={() => void navigator.clipboard.writeText(reference)}>{reference}</button>)}</div>{finding.unknowns.length > 0 && <div className="warning-box"><strong>Unknown information</strong><p>{finding.unknowns.join(" · ")}</p></div>}</aside>;
}

function GraphView({ blast, graph }: { blast: BlastResult | null; graph: { nodes: GraphNode[]; edges: GraphEdge[] } }) {
  ...
  }), [blast, reachableOnly, shortestOnly]);
  const visible = useMemo(() => graphFromBlast(filteredBlast), [filteredBlast]);
  const copyEvidence = (): void => { if (blast !== null) void navigator.clipboard.writeText(blast.findings.flatMap(({ evidenceRefs }) => evidenceRefs).join("\n")); };
  return <article className="panel graph-panel"><div className="panel-title"><div><p className="eyebrow">BOUNDED SERVER-SIDE SUBGRAPH</p><h2>Complete dependency evidence paths</h2></div><div className="legend"><span className="service-dot" />Service <span className="package-dot" />Package <span className="affected-dot" />Affected</div></div><div className="graph-controls"><label><input type="checkbox" checked={reachableOnly} onChange={(event) => setReachableOnly(event.target.checked)} />Reachable only</label><label><input type="checkbox" checked={shortestOnly} onChange={(event) => setShortestOnly(event.target.checked)} />Shortest path per finding</label><button className="secondary" onClick={copyEvidence}>Copy evidence IDs</button><span>All displayed nodes are affected-path evidence; the full graph never enters the browser.</span></div>{blast === null ? <Empty title="Nothing to graph" text="Run an incident analysis first." /> : <><GraphPanel nodes={visible.nodes} edges={visible.edges} /><div className="graph-footer"><span>{visible.nodes.length} visible nodes</span><span>{visible.edges.length} relationships</span><span>Maximum path depth 16</span><span>{blast.totalPaths} complete server-counted paths</span></div></>}</article>;
}
